...
def non_maximum_suppression(G, theta):
    ...
    for i in range(H):
        for j in range(W):
            ...
            #preserve or supress edge strength val
...
def link_edges(strong_edges, weak_edges):
    ...
    while len(pixels) > 0:
        ...
        for nrow, ncol in get_neighbors(row, col, H, W):
            if weak_edges[nrow, ncol]:
                edges[nrow,ncol] = True
                weak_edges[nrow, ncol] = False
                pixels.append((nrow, ncol))
    ### END YOUR CODE

    return edges

def canny(img, kernel_size=5, sigma=1.4, high=5, low=0):
