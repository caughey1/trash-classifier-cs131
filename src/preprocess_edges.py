"""
Batch edge detection preprocessor for TrashNet using Canny Edge Detector (Project 1).

Reads from:   data/dataset-resized/<class>/<file>.jpg
Writes to:    data/processed/<class>/<file>.jpg

Usage:
    python src/preprocess_edges.py
    python src/preprocess_edges.py --sigma 1.4 --high 5 --low 0
"""

from __future__ import annotations

import argparse
import sys
from multiprocessing import Pool, cpu_count
from pathlib import Path

import numpy as np
from PIL import Image
from tqdm import tqdm

from config import CLASS_NAMES, PROCESSED_DATA_DIR, RAW_DATA_DIR

sys.path.insert(0, str(Path(__file__).resolve().parent))
from edge import canny


def process_image(args: tuple) -> str | None:
    """Worker function — processes a single image. Returns path on success."""
    img_path, out_path, sigma, high, low = args

    if out_path.exists():
        return None

    try:
        img = Image.open(img_path).convert("L")
        img_array = np.array(img, dtype=np.float64)
        edges = canny(img_array, sigma=sigma, high=high, low=low)
        edge_uint8 = (edges * 255).astype(np.uint8)
        edge_rgb = np.stack([edge_uint8, edge_uint8, edge_uint8], axis=-1)
        Image.fromarray(edge_rgb).save(out_path)
        return str(out_path)
    except Exception as e:
        print(f"  ERROR on {img_path.name}: {e}")
        return None


def process_class(
    class_name: str,
    raw_root: Path,
    out_root: Path,
    sigma: float,
    high: float,
    low: float,
    workers: int,
) -> int:
    in_dir = raw_root / class_name
    out_dir = out_root / class_name
    out_dir.mkdir(parents=True, exist_ok=True)

    images = sorted(in_dir.glob("*.jpg")) + sorted(in_dir.glob("*.png"))
    if not images:
        print(f"  WARNING: no images found in {in_dir}")
        return 0

    job_args = [
        (img_path, out_dir / img_path.name, sigma, high, low)
        for img_path in images
    ]

    with Pool(processes=workers) as pool:
        list(tqdm(pool.imap(process_image, job_args), total=len(images), desc=class_name))

    return len(images)


def main() -> None:
    parser = argparse.ArgumentParser(description="CS131 Canny preprocessor for TrashNet.")
    parser.add_argument("--sigma",   type=float, default=1.4,        help="Gaussian sigma")
    parser.add_argument("--high",    type=float, default=5.0,        help="High threshold for strong edges")
    parser.add_argument("--low",     type=float, default=0.0,        help="Low threshold for weak edges")
    parser.add_argument("--raw-dir", type=Path,  default=RAW_DATA_DIR)
    parser.add_argument("--out-dir", type=Path,  default=PROCESSED_DATA_DIR)
    parser.add_argument(
        "--workers",
        type=int,
        default=cpu_count(),
        help=f"Parallel workers (default: all {cpu_count()} cores)",
    )
    args = parser.parse_args()

    if not args.raw_dir.exists():
        raise FileNotFoundError(
            f"Raw data not found at {args.raw_dir}. Run src/download_data.py first."
        )

    print(f"Input:   {args.raw_dir}")
    print(f"Output:  {args.out_dir}")
    print(f"Canny:   sigma={args.sigma}, high={args.high}, low={args.low}")
    print(f"Workers: {args.workers} cores\n")

    total = 0
    for class_name in CLASS_NAMES:
        count = process_class(
            class_name, args.raw_dir, args.out_dir,
            args.sigma, args.high, args.low, args.workers
        )
        print(f"  {class_name}: {count} images")
        total += count

    print(f"\nDone. {total} images written to {args.out_dir}")


if __name__ == "__main__":
    main()